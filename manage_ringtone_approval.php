<?php $page_title = "Manage Approval Ringtone";
    ...
    $targetpage = "manage_ringtone_approval.php"; 
    $limit = 15; 
    $keyword='';
    
    if(!isset($_GET['keyword'])){
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE tbl_ringtone.active='0'";
        
    } else{
        
        $keyword=addslashes(trim($_GET['keyword']));
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE (`ringtone_title` LIKE '%$keyword%' AND tbl_ringtone.active='0')";
        $targetpage = "manage_ringtone_approval.php?keyword=".$_GET['keyword'];
    }
    ...
    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_ringtone WHERE tbl_ringtone.active='0' ORDER BY tbl_ringtone.`id` DESC LIMIT $start, $limit"; 
    } else {
        $sql_query="SELECT * FROM tbl_ringtone WHERE tbl_ringtone.active='0' WHERE (`ringtone_title` LIKE '%$keyword%') ORDER BY tbl_ringtone.`id` DESC LIMIT $start, $limit"; 
    }
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center" >
                                            <div class="nsofts-switch enable_disable" data-bs-toggle="tooltip" data-bs-placement="top" title="Enable / Disable">
                                                <input type="checkbox" id="enable_disable_check_<?= $i ?>" data-id="<?= $row['id'] ?>" data-table="<?=$tableName ?>" data-column="active" class="cbx hidden btn_enable_disable" <?php if ($row['active'] == 1) { echo 'checked'; } ?>>
